resource "aws_cloudfront_distribution" "domain" {
  enabled             = true
  aliases             = ["web-${var.tags.Component}.${var.zone_name}"] # web-cdn.domain.com
  origin {
    domain_name = "web-${var.environment}.${var.zone_name}" # web_alb domain
    origin_id   = "web-${var.environment}.${var.zone_name}" # web-dev.domain.com
  ...
  default_cache_behavior {
    ...
    cached_methods         = ["GET", "HEAD", "OPTIONS"]
    target_origin_id       = "web-${var.environment}.${var.zone_name}" # web-dev.domain.com
    viewer_protocol_policy = "https-only"
